...
무기·적·웨이브·성장 요소를 전부 데이터 애셋으로 정의해 **코드 수정 없이 콘텐츠를 확장**할 수 있게 만들고, 각 시스템이 자기 책임만 갖도록 경계를 나눴습니다. 대량 처리가 필요한 잡몹과 개별 연출이 필요한 보스를 **서로 다른 표현 방식으로 나눠 구현**한 뒤, 그 선택이 옳았는지 직접 측정해 검증했습니다.
